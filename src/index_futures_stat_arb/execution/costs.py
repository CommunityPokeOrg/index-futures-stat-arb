...
class CostModel:
    ...
    equity_commission_per_share_usd: float = 0.005
    equity_fees_per_share_usd: float = 0.0
    equity_slippage_ticks: float = 1.0
    equity_half_spread_ticks: float = 0.5
    ...
    def fill_price(
        ...
        if product_spec.asset_class == "equity":
            distance = (
                self.equity_slippage_ticks + self.equity_half_spread_ticks
            ) * product_spec.tick_size
        else:
    ...
    def fees_usd(self, product_spec: ProductSpec, contracts: int) -> float:
        if product_spec.asset_class == "equity":
            return abs(contracts) * (
                self.equity_commission_per_share_usd + self.equity_fees_per_share_usd
            )
        return abs(contracts) * (
