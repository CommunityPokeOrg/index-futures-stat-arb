...
def test_kalman_level_tracks_unit_basis() -> None:
    rng = np.random.default_rng(12)
    x = rng.normal(9.0, 0.1, 500)
    level = KalmanLevel(delta=1e-5, obs_var=1e-4, alpha=2.0)
    estimates = []
    for value in 2.5 + rng.normal(0.0, 0.002, len(x)):
        level.predict()
        level.update(float(value))
        estimates.append(level.alpha)
    assert estimates[-1] == pytest.approx(2.5, abs=0.05)
    assert level.alpha is not None
...
def test_unit_simulation_has_no_future_dependency() -> None:
    bars = make_bars(sessions=8, bars_per_session=6)
    cfg = SimulationConfig(
        start="2026-01-05",
        end="2026-01-20",
